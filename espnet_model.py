...
from espnet2.layers.fusion import iAFF
...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        speech2: torch.Tensor,
        speech_lengths2: torch.Tensor,
        text: torch.Tensor,
        ...
        **kwargs,
        ...
        # 1. Encoder
        encoder_out, encoder_out_lens = self.encode(speech, speech_lengths, speech2, speech_lengths2)
    ...
    def encode(
        ...
        with autocast(False):
            # 1. Extract feats
            feats, feats_lengths = self._extract_feats(speech, speech_lengths) #enh
            feats2, feats_lengths2 = self._extract_feats(speech2, speech_lengths2) #org
            #torch.save(feats2, '/work/clean_feats.pt')

            # 2. Data augmentation
            if self.specaug is not None and self.training:
                feats, feats_lengths = self.specaug(feats, feats_lengths)
                feats2, feats_lengths2 = self.specaug(feats2, feats_lengths2)
            ...
            if self.normalize is not None:
                ...
                feats2, feats_lengths2 = self.normalize(feats2, feats_lengths2)


        # Pre-encoder, e.g. used for raw input data
        if self.preencoder is not None:
            feats, feats_lengths = self.preencoder(feats, feats_lengths)
            feats2, feats_lengths2 = self.preencoder(feats2, feats_lengths2)


        torch.save(feats, '/work/enh_feats.pt')
        torch.save(feats2, '/work/org_feats.pt')
        fusion_feats = self.iaff.forward(feats, feats2)
        #保存为张量
        torch.save(fusion_feats, '/work/fusion_feats.pt')
        ...
        if self.encoder.interctc_use_conditioning:
            encoder_out, encoder_out_lens, _ = self.encoder(
                fusion_feats, feats_lengths, ctc=self.ctc
            )
        else:
            encoder_out, encoder_out_lens, _ = self.encoder(fusion_feats, feats_lengths)
        intermediate_outs = None
        if isinstance(encoder_out, tuple):
            intermediate_outs = encoder_out[1]
            encoder_out = encoder_out[0]
