...
class Corrector(Detector):
    ...
    def correct(self, text, include_symbol=True, num_fragment=1, threshold=57, **kwargs):
        ...
        for sentence, idx in sentences:
            maybe_errors, proper_details = self.detect_sentence(sentence, idx, **kwargs)    #  检测初始入口
        ...
        if not details == []:
            flag = True
            for i in range(ld):
                matric_detals.append([details[i][1], details[i][2], details[i][3]])

            return text_new, details, matric_detals, ld, flag    # 加了 details[][]
        else:
            flag = False
            return text_new, details, flag
